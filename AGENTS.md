...
### Progress
#### Done
- **CrazyGames SDK base launch (099)** *(implementado, verificación pendiente en Unity)*: integración base del SDK de CrazyGames v5.33.0 (paquete `UnityCrazySDK.unitypackage` → `Assets/CrazySDK`). Nuevo `Assets/Scripts/Game/CrazySDKIntegration.cs` (estático, patrón `GamanbitBootstrap`): `Boot()` → `CrazySDK.Init` guardado con `CrazySDK.IsAvailable` (inmune a EXE/Android, donde el SDK lanza excepción fuera de WebGL/Editor), registra `AddSettingsChangeListener` y aplica `muteAudio` del jugador a `AudioListener.volume`; `GameplayStart()/GameplayStop()` con guard `gameplayStarted` (idempotentes) y `HappyTime()`. Hooks: `MainMenuManager.Start` → `Boot()`; `GameManager.Start` → `GameplayStart()` (arranque de partida en SampleScene); `GameOverUI.Show` → `GameplayStop()` + `HappyTime()` en victoria azul. `Assembly-CSharp.csproj` incluye el nuevo archivo. Monetización (ads/rewarded/banners), template WebGL de CrazyGames y build vía su builder quedan pendientes. Validación: dotnet build Assembly-CSharp.csproj = 0 errores (warnings solo de SDK).
